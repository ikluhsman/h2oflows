<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
    <AppHeader>
      ...
      <span class="text-sm font-medium text-gray-700 dark:text-gray-200">My Reports</span>
    </AppHeader>
    ...
    <main v-else class="max-w-3xl mx-auto px-4 py-6 space-y-5">
      <div class="flex items-center justify-between">
        ...
        <NuxtLink
          to="/reports/new"
          class="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 px-3 py-1.5 text-sm font-medium text-white transition-colors"
        >
      ...
      <!-- Empty -->
      <div v-else-if="reports.length === 0" class="mt-16 flex flex-col items-center gap-3 text-center text-gray-400">
        ...
        <NuxtLink to="/reports/new" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">File your first report</NuxtLink>
      </div>
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :key="rep.id"
          class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 space-y-2"
        >
          <!-- Top row: name + date -->
          <div class="flex items-start justify-between gap-3">
            ...
            <span class="text-xs text-gray-400 shrink-0">{{ formatDate(rep.report_date) }}</span>
          </div>

          <!-- Content snippet -->
          <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{{ rep.content }}</p>

          <!-- Flow badge -->
          <div v-if="rep.flow_cfs != null" class="flex items-center gap-1.5">
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ Math.round(rep.flow_cfs).toLocaleString() }} cfs</span>
      ...
      <Teleport to="body">
        ...
        >
          <div class="w-full max-w-sm bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl p-6 space-y-4">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Delete report?</h3>
