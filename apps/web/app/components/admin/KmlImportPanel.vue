  ...
  <div class="space-y-4">
    <div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          class="text-sm text-primary-500 hover:text-primary-400 font-medium transition-colors"
    ...
    <!-- KML Format Guide -->
    <div v-if="showKmlGuide" class="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg px-4 py-4 text-xs text-neutral-600 dark:text-neutral-400 space-y-4">
      <div>
      ...
      <div>
        <p class="font-semibold text-neutral-700 dark:text-neutral-200 mb-1">Metadata placemarks (coordinate-less)</p>
        <p class="text-neutral-400">Keys: <code>common_name</code>, <code>description</code>, <code>min_class</code>, <code>max_class</code>, <code>gauge</code>, <code>permit_required</code>, <code>multi_day</code></p>
        <p class="mt-1 text-neutral-400">Flow bands: <code>below</code> (upper Too Low CFS), <code>running</code> (min,max), <code>high</code> (min,max), <code>above</code> (lower Very High CFS)</p>
        <p class="mt-1 text-neutral-400">Pin prefixes: <code>Rapid:</code>, <code>Wave:</code>, <code>Put-in:</code>, <code>Take-out:</code>, <code>Parking:</code>, <code>Campsite:</code>, <code>Hazard:</code></p>
      </div>
    </div>
    ...
    <div v-if="importLog.length > 0">
      <div class="flex items-center justify-between mb-2">
        <p class="text-xs font-semibold text-neutral-500 uppercase tracking-wide">Import log</p>
        <button class="text-xs text-neutral-400 hover:text-neutral-600 transition-colors" @click="importLog = []">Clear</button>
      </div>
      <div class="bg-neutral-950 rounded-lg border border-neutral-800 px-4 py-3 max-h-72 overflow-y-auto font-mono text-[11px] space-y-0.5">
        <p
          v-for="(line, i) in importLog"
          :key="i"
          :class="line.startsWith('✗') || line.startsWith('⚠') ? 'text-red-400' : line.startsWith('+') ? 'text-emerald-400' : line.startsWith('✓') ? 'text-neutral-300' : 'text-neutral-500'"
        >{{ line }}</p>
      </div>
    </div>
  </div>
