<template>
  <div
    class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-4 cursor-pointer hover:border-primary-200 dark:hover:border-primary-800 hover:shadow-sm transition-all"
    @click="emit('open')"
  >
    ...
    <div class="flex items-start justify-between gap-2 mb-3">
      ...
      <span
        ...
        class="shrink-0 text-xs font-medium text-primary-500 bg-primary-50 dark:bg-primary-950 rounded-full px-2 py-0.5"
      >Shared</span>
      <span
        v-else-if="trip.share_consent === false"
        class="shrink-0 text-xs font-medium text-neutral-400 bg-neutral-100 dark:bg-neutral-800 rounded-full px-2 py-0.5"
      >Private</span>
    </div>
    ...
    <!-- Notes preview -->
    <p v-if="trip.notes" class="mt-2 text-xs text-neutral-500 dark:text-neutral-400 line-clamp-2">{{ trip.notes }}</p>
  </div>
</template>
