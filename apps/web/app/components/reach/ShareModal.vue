  ...
  <UModal v-model:open="open" :ui="{ width: 'max-w-lg' }">
    <template #header>
      <div class="flex items-center justify-between gap-2 w-full">
        <div>
          ...
          <p class="text-xs text-neutral-400 mt-0.5">{{ reachName }}</p>
        </div>
        <button
          v-if="step === 'form'"
    ...
    <template #body>
      ...
      <div v-if="step === 'pick'" class="space-y-3">
        ...
        <div class="grid grid-cols-3 gap-2">
          <button
            ...
            :key="dest.id"
            class="flex flex-col items-center gap-1.5 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 hover:bg-primary-50 dark:hover:bg-primary-950/30 hover:border-primary-300 dark:hover:border-primary-700 px-3 py-3 text-center transition-colors"
            @click="selectDest(dest.id)"
          >
            <span class="text-2xl">{{ dest.icon }}</span>
            <span class="text-xs font-medium text-neutral-700 dark:text-neutral-200 leading-tight">{{ dest.label }}</span>
            <span class="text-[10px] text-neutral-400 leading-tight">{{ dest.sublabel }}</span>
          </button>
        </div>

        <!-- Coming-soon external destinations -->
        <div>
          <p class="text-[10px] text-neutral-400 uppercase tracking-wide mb-2">Also share to (coming soon)</p>
          <div class="flex flex-wrap gap-2">
            <div
              v-for="ext in externalDests"
              :key="ext.id"
              class="flex items-center gap-1.5 rounded-lg border border-neutral-100 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 px-3 py-1.5 opacity-50 cursor-not-allowed"
            >
              <span class="text-base">{{ ext.icon }}</span>
              <span class="text-xs text-neutral-500">{{ ext.label }}</span>
            </div>
          </div>
        </div>
        ...
        <label class="flex items-start gap-2.5 cursor-pointer pt-1">
          <input
            ...
            type="checkbox"
            class="mt-0.5 rounded accent-primary-600"
          />
          <div>
            <span class="text-xs font-medium text-neutral-700 dark:text-neutral-200">Share anonymized data with h2oflows</span>
            <p class="text-[11px] text-neutral-400 mt-0.5">Helps improve flow recommendations · No personal data shared</p>
          </div>
        </label>
      ...
      <div v-else-if="step === 'form' && selectedDest === 'trip_report'" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Title <span class="text-neutral-400">(optional)</span></label>
          <input
            v-model="formTitle"
            type="text"
        ...
        <div>
          <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Notes</label>
          <textarea
            v-model="formBody"
            rows="4"
            placeholder="How was the run? Any hazards, lines, or beta worth sharing?"
            class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/40 resize-none"
          />
        </div>

        <div class="flex gap-3">
          <div class="flex-1">
            <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Flow impression</label>
          ...
          <div class="w-40">
            ...
            <input
              v-model="formDate"
              type="date"
              class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/40"
            />
          </div>
        </div>

        <!-- Current CFS -->
        <div v-if="currentCfs != null" class="flex items-center gap-2 rounded-lg bg-neutral-50 dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 px-3 py-2">
          <svg class="w-3.5 h-3.5 text-neutral-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
          <span class="text-xs text-neutral-500">Will record current flow: <strong class="text-neutral-700 dark:text-neutral-200">{{ currentCfs.toLocaleString() }} cfs</strong>
            <span v-if="flowStatus" class="ml-1 text-neutral-400">({{ flowStatusLabel(flowStatus) }})</span>
          </span>
        </div>

        <div class="flex items-center gap-2 pt-1">
          <label class="flex items-center gap-2 cursor-pointer flex-1">
            <input v-model="formPublish" type="checkbox" class="rounded accent-primary-600" />
            <span class="text-xs text-neutral-600 dark:text-neutral-400">Publish publicly</span>
          </label>
          <label class="flex items-center gap-2 cursor-pointer">
            <input v-model="shareConsent" type="checkbox" class="rounded accent-primary-600" />
            <span class="text-xs text-neutral-500">Share with h2oflows</span>
          </label>
        </div>
      ...
      <div v-else-if="step === 'form' && selectedDest !== 'trip_report'" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">
            {{ selectedDest === 'hazard_alert' ? 'Describe the hazard' : 'Note' }} <span class="text-neutral-400">(optional)</span>
          </label>
          <textarea
            v-model="formBody"
            rows="3"
        ...
        <div v-if="selectedDest === 'flow_update'" class="flex gap-3">
          ...
          <div class="w-40">
            <label class="block text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1">Date</label>
            <input v-model="formDate" type="date" class="w-full rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/40" />
          </div>
        </div>
