  ...
  >
    ...
    >
      <div class="flex items-center gap-2">
        <div class="flex items-center gap-1 min-w-0 flex-1">
          <span
            class="min-w-0 font-semibold text-gray-800 dark:text-gray-100 truncate"
            :class="density === 'compact' ? 'text-sm' : 'text-sm'"
          >{{ reachLabel(reach) }}</span>
          <NuxtLink
            :to="`/reaches/${reach.contextReachSlug}`"
            class="shrink-0 p-0.5 rounded text-gray-300 dark:text-gray-600 hover:text-blue-500 dark:hover:text-blue-400 transition-colors"
            aria-label="View reach page"
        ...
        >{{ displayCfs(reach) != null ? displayCfs(reach)!.toLocaleString() : '—' }}</span>
